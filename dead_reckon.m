...
if any([min(size(speed)) min(size(time))] ~= 1) || any([ndims(speed) ndims(time)] > 2)
    error(['map:' mfilename ':mapError'], 'Speed and time must be a scalar or vector')
end
...
if isempty(spdtimes) % derived speed times
    ...
    elseif length(speed) == (size(lat, 1) - 1) % you have a speed for each leg
        ...
        error(['map:' mfilename ':mapError'], 'Speed must be a scalar or matched to times or track legs')
    end
...
elseif any(size(speed) ~= size(spdtimes))  % you input speed times
        error(['map:' mfilename ':mapError'], 'To be matched, speeds and times must be of same size')
else
